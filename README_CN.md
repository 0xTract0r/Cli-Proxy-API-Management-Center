...
- **API Keys**：管理代理 `api-keys`（增/改/删）。可视化配置页里的新增、编辑、删除会立即通过管理端 `/api-keys` 接口持久化；其他配置项仍走页面级保存流程。
...
- **OAuth**：引导式账号接入，对 Codex、Anthropic/Claude、Antigravity、Gemini CLI、Kimi、xAI/Grok 发起 OAuth/设备码流程并轮询状态；支持提交回调 URL 或 xAI/Grok 页面显示的 code；包含 Vertex JSON 凭据导入；登录成功后可直接查看生成的认证文件。
- **配额管理**：管理 Claude、Antigravity、Codex、Gemini CLI、xAI/Grok 等提供商的配额上限与使用情况；Codex/Claude 额度由 core 统一定时刷新并持久化，页面进入时只读取最近快照，手动“立即刷新”会通过 core 的 `/v0/management/quota/refresh` 触发刷新。
- **使用统计**：按小时/天图表、按 API 与按模型统计、缓存/推理 Token 拆分、按当前时间范围计算平均 RPM/TPM、可选本地保存的模型价格用于费用估算。
